...
class Light extends BaseGauge {
    ...
    draw() {
        ...
        this.ctx.save();
        this.ctx.beginPath();
        this.ctx.rect(this.width/4, this.height-this.height/4, this.width, -this.height * normalizedValue/ 100);
        this.ctx.closePath();
        this.ctx.clip();

        // Draw the fill shape of the sun
        this.ctx.beginPath();
        this.circle();
        this.ctx.fillStyle = 'rgba(255, 255, 0, .4)';
        this.ctx.fill();
        this.ctx.closePath();
        this.ctx.restore();

        // Draw the border of the sun
        this.ctx.beginPath();
        this.circle();
        this.ctx.lineWidth = this._lineWidth;
        this.ctx.strokeStyle = 'rgba(255, 255, 0, 1)';
        ...
        this.ctx.textAlign = 'center';
        this.ctx.textBaseline = 'middle';
        this.ctx.fillText(Math.round(this._value * 100) / 100 , this.width / 2, this.height / 2);
        this.ctx.fillText(this.unit, this.width / 2, this.height / 2 + this.width / 9);
    }

    circle() {
        this.ctx.arc(this.width / 2, this.height / 2, this.width / 4, 0, Math.PI * 2);
    }
    ...
    rays() {
        ...
        for (let i = 0; i < 360; i += 360 / 12) {
            this.ctx.moveTo(this.width / 2 + ((this.width/2 ) +10) * Math.cos(i * Math.PI / 180), this.height / 2 + ((this.height/2 )+10) * Math.sin(i * Math.PI / 180));
            this.ctx.lineTo(this.width / 2 + (this.width / 3) * Math.cos(i * Math.PI / 180), this.height / 2 + (this.height / 3) * Math.sin(i * Math.PI / 180));
        }
        this.ctx.stroke();
        this.ctx.closePath();
    }
...
customElements.define('light-gauge', Light, { extends: 'canvas' });

export { MultiColoredGauge, Humidity, Temperature, GaugeColors, NotConnected, Bolt, Light };
